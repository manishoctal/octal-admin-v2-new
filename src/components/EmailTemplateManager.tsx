...
import { Edit, Eye, MoreHorizontal, Shield, UserCheck, UserX } from 'lucide-react';
...
export function EmailTemplateManager() {
  ...
  const loadSubadmins = async () => {
    try {
      ...
      const resp = await apiGet(apiPath.emailTemplate,payload);
  ...
  const handleViewSubadmin = (subadminId: string) => {
    navigate(`/email-template/view`, {state:subadminId });
  };

  const handleEditSubadmin = (subadminId: string) => {
    navigate(`/email-template/edit`, {state:subadminId });
  ...
  const canView = hasPermission(MODULES.EMAIL_TEMPLATE, ACTIONS.VIEW);
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold">Email Template</h2>
