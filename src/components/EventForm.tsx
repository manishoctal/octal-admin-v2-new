...
export function EventForm({ mode }: CircleFormProps) {
  const navigate = useNavigate();
  const { user } = useAuth();
  const location = useLocation()
  const circle = location?.state
  const isAdd = location?.pathname == '/events/create'
  const isEdit = location?.pathname == '/events/edit'
  const isView = location?.pathname == '/events/view'
  ...
  const [value, setValue] = React.useState<string[]>(location?.state?.circleId || '');
  const [intentData, setIntentData] = useState([])

  const [formData, setFormData] = useState<ProfileFormData>({
    name: location?.state?.name || '',
    avatar: location?.state?.image || '',
  ...
  const addCircle = async (payload) => {
    let resp
    try {
      resp = isEdit ? await apiPut(apiPath.getEvents + '/' + circle?._id, payload) : await apiPost(apiPath.getEvents, payload)
    ...
    try {
      ...
      if (file) {
        ...
        imageReader.onloadend = async () => {
          ...
          if (resp?.status === 200) {
            addCircle({ name: formData?.name, venue: formData?.venue, city: formData?.city, circleId: value, shortDescription: formData?.shortDescription, startDate: new Date(formData?.startDate), endDate: new Date(formData?.endDate), image: key })
          }
        };
        return
      }
      addCircle({ name: formData?.name, venue: formData?.venue, city: formData?.city, circleId: value, shortDescription: formData?.shortDescription, startDate: new Date(formData?.startDate), endDate: new Date(formData?.endDate), image: undefined })
  ...
  const handleCancel = () => {
    setFormData({ name: '', avatar: '', venue: '', startDate: '', endDate: '', city: '', shortDescription: '' });
    setError('');
    setFile('')
    setValue('')
  };
  ...
  const hasChanges = () => {
    ...
    const circleNotEmpty = value?.length > 0;
    const shortDescriptionNotEmpty = formData?.shortDescription?.trim() !== "";
    const venueDescriptionNotEmpty = formData?.venue?.trim() !== "";
    const startDateNotEmpty = formData?.startDate !== "";
    const endDateNotEmpty = formData?.endDate !== "";
    const avatarNotEmpty = formData?.avatar?.trim() !== "";
    const nameChanged = formData?.name !== originalData?.name;
    const circleChanged = value !== location?.state?.circleId
    const avatarChanged = formData?.avatar !== originalData?.avatar;
    const cityChanged = formData?.city !== originalData?.city;
    const venueChanged = formData?.venue !== originalData?.venue;
    const shortDescriptionChanged = formData?.shortDescription !== originalData?.shortDescription;
    const startDateChanged = formData?.startDate !== originalData?.startDate;
    const endDateChanged = formData?.endDate !== originalData?.endDate;
    return endDateNotEmpty && startDateNotEmpty && cityNotEmpty && circleNotEmpty && venueDescriptionNotEmpty && shortDescriptionNotEmpty && nameNotEmpty && avatarNotEmpty && (nameChanged || avatarChanged || circleChanged || cityChanged || venueChanged || shortDescriptionChanged || startDateChanged || endDateChanged);
  };
  ...
  const getIntent = async () => {
    try {
      const resp = await apiGet(apiPath.getAllCircle)
      if (resp?.data?.success) {
        const filterData = resp?.data?.results?.docs?.map((res) => { return ({ label: res?.name, value: res?._id }) })
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <div>
            <h1 className="text-3xl font-bold">
              {isAdd ? 'Create New Event' : isEdit ? 'Edit Event' : 'View Event'}
            </h1>
            <p className="text-muted-foreground">
              {isAdd ? 'Create a new event for users to join and engage' : isEdit ? 'Update event information and settings' : 'View event information'}
        ...
        {isView && (
          <PermissionGate module={MODULES.EVENTS} action={ACTIONS.EDIT}>
            <Button onClick={() => navigate(`/events/edit`, { state: { ...circle } })} className="gap-2">
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6 mt-3">
                ...
                <div className="space-y-4 mt-2 ">
                  ...
                  {!isEditing && formData?.avatar && (
                    <p className="text-sm text-muted-foreground">
                      Event image set • Click "Edit Event" to change
                    </p>
                  )}
                </div>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-2'>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="intent">Circle<span className='text-red-500'>*</span></Label>
                    <SingleSelect
                      options={intentData}
                      value={value}
                      onChange={setValue}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="name">Venue<span className='text-red-500'>*</span></Label>
                    <Input
                      id="name"
                      type="text"
                    ...
                    <div className="flex justify-between text-xs text-muted-foreground">
                      ...
                      <span>{formData?.venue?.length}/150</span>
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  <div className='flex sm:gap-5 gap-1 w-full '>
                    <div className='sm:mb-0 mb-4'>
                      <Label htmlFor="startDate" className='mb-2'>Start Date<span className='text-red-500'>*</span></Label>
                      <DatePicker
                        id='startDate'
                        value={formData?.startDate}
                        minDate={startOfDay(subDays(new Date(), 0))}
                        maxDate={new Date(formData?.endDate)}
                        onChange={(e) => setFormData(prev => ({ ...prev, startDate: e || '' }))}
                        placeholder="Select Start Date"
                        disabled={isView || loading}
                      />
                      <div className="flex justify-between text-xs text-muted-foreground">
                        <span>Display start date</span>
                      </div>

                    </div>

                    <div>
                      <Label htmlFor="endDate" className='mb-2'>Start Date<span className='text-red-500'>*</span></Label>
                      <DatePicker
                        id='endDate'
                        value={formData?.endDate}
                        minDate={new Date(formData?.startDate)}
                        onChange={(e) => setFormData(prev => ({ ...prev, endDate: e || '' }))}
                        disabled={isView || loading}
                        placeholder="Select End Date"
                      />
                      <div className="flex justify-between text-xs text-muted-foreground">
                        <span>Display end date</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <CardContent className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>{helpers.capitalizeFirstWord(circle?.type)}</span>
                </div>
              </div>
